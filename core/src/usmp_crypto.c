...
int usmp_gcm_encrypt(const uint8_t *key, uint32_t seq,
  ...
  if (mbedtls_gcm_crypt_and_tag(&gcm, MBEDTLS_GCM_ENCRYPT, plain_len, nonce,
                                USMP_GCM_NONCE_LEN, aad, aad_len, plaintext,
                                out, USMP_GCM_TAG_LEN, out + plain_len) != 0)
    goto done;
...
int usmp_gcm_decrypt(const uint8_t *key, uint32_t seq,
  ...
  if (mbedtls_gcm_auth_decrypt(&gcm, cipher_len, nonce, USMP_GCM_NONCE_LEN, aad,
                               aad_len, tag, USMP_GCM_TAG_LEN,
