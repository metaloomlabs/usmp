...
/*
 * Precomputed CRC-16-ANSI / CRC-16-IBM lookup table (polynomial 0xA001 reflected).
 * Stored in const .rodata (Flash ROM only, 512 bytes, 0 bytes RAM).
 * Provides ~8x faster single-cycle per-byte calculation compared to bitwise shifting loops.
 */
static const uint16_t usmp_crc16_table[256] = {
...
/* Fast table-driven CRC-16-ANSI step processor */
static inline uint16_t usmp_crc16_step(uint16_t crc, const uint8_t* data, uint16_t len) {
  for (uint16_t i = 0; i < len; i++) {
    crc = (uint16_t)((crc >> 8) ^ usmp_crc16_table[(crc ^ data[i]) & 0xFF]);
  }
  return crc;
}
